        ...
        <FullPage>
            <j-card>
                <j-row :gutter="[24, 24]" style="padding: 24px">
                    <j-col :span="12">
                        <j-form ref="formRef" :label-col="{ span: 8 }" :model="formData" :wrapper-col="{ span: 24 }" autocomplete="off"
                            class="form" layout="vertical" name="basic">
                            <j-form-item :rules="[{ required: true, message: '请选择证书标准', trigger: 'blur' }]" label="证书标准"
                                name="type">
                            ...
                            <j-form-item :rules="[
                                ...
                                <j-input v-model:value="formData.name" placeholder="请输入证书名称" />
                            </j-form-item>
                            <j-form-item :name="['configs','cert']" :rules="[
                                { required: true, message: '请输入或上传文件', trigger: 'blur' },
                            ]" label="证书文件">
                                <CertificateFile v-model:modelValue="formData.configs.cert" name="cert"
                                    placeholder="请输入证书文件" />
                            </j-form-item>
                            <j-form-item :rules="[{ required: true, message: '请选择证书类型', trigger: 'blur' }]" label="证书类型"
                                name="mode">
                                <j-radio-group v-model:value="formData.mode" button-style="solid">
                                    <j-radio-button size="large" style="margin-right: 30px;"
                                        value="client">客户端</j-radio-button>
                                    <j-radio-button size="large" value="server">服务端</j-radio-button>
                                </j-radio-group>
                            </j-form-item>
                            ...
                            <j-form-item v-if="formData.mode !== 'client'" :name="['configs','key']" :rules="[
                                { required: true, message: '请输入或上传文件', trigger: 'blur' },
                            ]" label="证书私钥">
                                <CertificateFile v-model:modelValue="formData.configs.key" name="key"
                                    placeholder="请输入证书私钥" />
                            </j-form-item>
                            <j-form-item :rules="[{ max: 200, message: '最多可输入200个字符' }]" label="说明" name="description">
                                <j-textarea v-model:value="formData.description" :maxlength="200" :rows="3"
                                    placeholder="请输入说明" showCount />
                            </j-form-item>

                            <j-form-item>
                                <j-button v-if="view === 'false'" :loading="loading" class="form-submit" html-type="submit"
                                    type="primary" @click.prevent="onSubmit">保存</j-button>
                            </j-form-item>
                        </j-form>
                    </j-col>
                    <j-col :span="12">
