        ...
        <j-row :gutter="24">
            <j-col :span="12">
                ...
                >
                    <j-form-item
                        name="id"
                        :label="$t('DeviceAccess.index.006446-0')"
                        :rules="[{ required: true, message: $t('DeviceAccess.index.006446-1') }]"
                    >
                        <j-select
                            v-model:value="productData.id"
                            :options="productTypes"
                            @change="productTypeChange"
                            :placeholder="$t('DeviceAccess.index.006446-1')"
                        />
                    </j-form-item>
                </j-form>
                <!--        其它接入配置        -->
                <div v-for="(i,index) in metadata">
                    <Title v-if="i?.name" :data="i?.name" class="config">
                        <template #extra>
                            <j-tooltip
                                :title="$t('DeviceAccess.index.006446-2')"
                            >
                                <AIcon
                            ...
                            :rules="[
                                {
                                    ...
                                    message: `${
                                        item.type.type === 'enum' || 'boolean'
                                            ? $t('DeviceAccess.index.006446-3')
                                            : $t('DeviceAccess.index.006446-4')
                                    }${item.name}`,
                                },
                            ]"
                        >
                            <j-input
                                :placeholder="$t('DeviceAccess.index.006446-4')"
                            ...
                            ></j-input-password>
                            <j-select
                                :placeholder="$t('DeviceAccess.index.006446-3')"
                                v-if="
                                    item.type.type === 'enum' ||
                ...
                <Title :data="$t('DeviceAccess.index.006446-5')">
                    <template #extra>
                        <j-tooltip
                ...
                <PermissionButton
                    ...
                    >{{ $t('DeviceAccess.index.006446-6') }}</PermissionButton
                >
            </j-col>
            <j-col
                :span="12"
...
const handleColumns = () => {
    const Group = {
        title: $t('DeviceAccess.index.006446-7'),
        dataIndex: 'group',
        key: 'group',
        ellipsis: true,
        align: 'center',
    ...
    if (metadata.value.length) {
        metadata.value.forEach((i: any) => {
            i?.properties.forEach((item: any) => {
                if (
                    item.name === $t('DeviceAccess.index.006446-8') &&
...
const updateAccessData = async (id: string, values: any) => {
    ...
    if (resp.status === 200) {
        onlyMessage($t('DeviceAccess.index.006446-9'));
...
const tooltip = computed(() => {
    if (productStore.current?.count > 0) {
        return $t('DeviceAccess.index.006446-10');
    }
    if (productStore.current.state === 1) {
        return $t('DeviceAccess.index.006446-11');
    }
    return '';
});
