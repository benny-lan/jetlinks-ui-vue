<template>
  ...
  >
    <template v-if="visible" #content>
      <j-scrollbar v-if="showMetrics || config.length > 0"   height="350">
        <j-collapse v-model:activeKey="activeKey">
          ...
          <j-collapse-panel v-if="showMetrics" key="metrics">
            <template #header>
              指标配置
              <j-tooltip title="场景联动页面可引用指标配置作为触发条件">
                <AIcon style="padding-left: 12px;padding-top: 4px;" type="ExclamationCircleOutlined" />
              </j-tooltip>
            </template>
...
<script lang="ts" name="OtherSetting" setup>
