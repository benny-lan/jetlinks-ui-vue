                    ...
                    <template #id="{ record, index }">
                        <EditTableFormItem :name="[index, 'id']">
                            <a-input
                                ...
                                :placeholder="$t('Object.index.843741-0')"
                            />
                        </EditTableFormItem>
                    </template>
                    <template #name="{ record, index }">
                        <EditTableFormItem :name="[index, 'name']">
                            <a-input
                                ...
                                :placeholder="$t('Object.index.843741-1')"
                            />
                        </EditTableFormItem>
                    </template>
...
const validatorConfig = (value, _isObject = false) => {
    ...
    if (value.type === 'enum' && !value.elements?.length) {
        return Promise.reject($t('Object.index.843741-3'));
    }
    if (value.type === 'array' && !value.elementType?.type) {
        return Promise.reject($t('Object.index.843741-4'));
    }
    ...
    if (value.type === 'date' && !value.format) {
        return Promise.reject($t('Object.index.843741-5'));
    }
    ...
    if (
        ...
        return Promise.reject($t('Object.index.843741-6'));
    }
...
const defaultColumns = [
            ...
            rules: [
                {
                    asyncValidator(_, value, ...setting) {
                        if (value) {
                            ...
                            if (isSome) {
                                return Promise.reject($t('Object.index.843741-8'));
                            }
                            return Promise.resolve();
                        }
                        return Promise.reject($t('Object.index.843741-0'));
                    },
                },
                { max: 64, message: $t('Object.index.843741-9') },
                {
                    pattern: /^[a-zA-Z0-9_]+$/,
                    message: $t('Object.index.843741-10'),
                },
            ],
        },
    },
    {
        title: $t('Object.index.843741-11'),
        dataIndex: 'name',
        form: {
            rules: [
                {
                    required: true,
                    message: $t('Object.index.843741-1'),
                },
                { max: 64, message: $t('Object.index.843741-9') },
            ],
        },
    ...
    props.type === 'functions'
        ? {
              title: $t('Object.index.843741-12'),
              dataIndex: 'expands',
              width: 120,
          }
        : null,
    {
        title: $t('Object.index.843741-13'),
        dataIndex: 'valueType',
        width: 240,
        form: {
            required: true,
            rules: [
                {
                    validator(_, value) {
                        if (!value?.type) {
                            return Promise.reject($t('Object.index.843741-14'));
                        }
...
const myColumns = computed(() => {
    return [
        ...
        {
            dataIndex: 'action',
            title: $t('Object.index.843741-15'),
            width: 60,
        },
    ];
});
