<template>
  <j-popconfirm-modal bodyStyle="width: 450px;height: 300px" @confirm="confirm">
    <template #content>
      <j-scrollbar>
        <j-form ref="formRef" :model="formData" layout="vertical">
          ...
          <j-form-item :name="['virtualRule', 'windowType']" label="窗口" required>
            <j-select
          ...
          <template v-if="showWindow">
            <j-form-item :name="['virtualRule', 'aggType']" label="聚合函数">
              <j-select
...
<script lang="ts" name="Rule" setup>
