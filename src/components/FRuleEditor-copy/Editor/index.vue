      ...
      <div class="right">
        <span v-if="mode !== 'advance'">
          <j-tooltip :title="!id ? '请先输入标识' : '设置属性规则'">
            <AIcon :class="!id ? 'disabled' : ''" type="FullscreenOutlined" @click="fullscreenClick" />
          </j-tooltip>
        </span>
      </div>
    </div>
    <div class="editor">
      <j-monaco-editor v-if="loading" ref="editor" v-model:model-value="_value" language="javascript" theme="vs"/>
    </div>
  </div>
</template>
