<template>
  <div class="operator-box">
    <j-input-search allow-clear placeholder="搜索关键字" @search="search" />
    <div class="tree">
      <j-tree :field-names="{ title: 'name', key: 'id', }" :show-icon="true" :showLine="{ showLeafIcon: false }"
        :tree-data="data"
        auto-expand-parent
        @select="selectTree">
        <template #title="node">
                ...
                <template #content>
                  <j-space direction="vertical">
                    ...
                    <j-tooltip placement="right" title="实时值的上一有效值">
                      <j-button type="text" @click="lastClick(node)">
                        上一值
                      </j-button>
                    </j-tooltip>
                  </j-space>
...
<script lang="ts" name="Operator" setup>
