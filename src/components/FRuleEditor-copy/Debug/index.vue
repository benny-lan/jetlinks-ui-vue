<template>
  <div class="debug-container">
    <div class="left">
      ...
      <j-table :columns="columns" :data-source="property" :pagination="false" bordered size="small">
        <template #bodyCell="{ column, record, index }">
          <template v-if="column.key === 'id'">
            <j-auto-complete v-model:value="record.id" :options="options" size="small" style="width: 130px" />
          </template>
      ...
      <j-button block style="margin-top: 5px" type="dashed" @click="addItem">
        <template #icon>
    ...
    <div class="right">
      ...
      <div class="log">
        <j-descriptions>
          <j-descriptions-item v-for="item in ruleEditorStore.state.log" :key="item.time"
            :label="moment(item.time).format('HH:mm:ss')" :span="3">
            <j-tooltip :title="item.content" placement="top">
              {{ item.content }}
            </j-tooltip>
          </j-descriptions-item>
        </j-descriptions>
...
<script lang="ts" name="Debug" setup>
